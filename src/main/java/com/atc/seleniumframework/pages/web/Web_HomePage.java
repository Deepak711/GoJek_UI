...
public class Web_HomePage {
	...
	public @FindBy(xpath="//div[@id='app']//a[@class='button-main-content']//span[text()='Continue']")
	...
	public @FindBy(xpath="//span[text()='Invalid card number']")
	WebElement msg_InvalidCCNum;
	...
	public @FindBy(xpath="//label[text()='Midtrans Promo']")
	WebElement chk_Promo;
	
	public @FindBy(xpath="//div[@class='text-button-main']")
	WebElement btn_Paynow;
	
	public @FindBy(xpath="//*[@id='acsForm']//*[@class='col-xs-7']//*[@type='password']")
	WebElement txt_OTP;
	...
	public void purchasepillow(String vCNum, String vExpiryDate, String vCVV, String vOTP) throws InterruptedException {
		...
		if(GeneralUtils.CheckElementdisplayed(btn_Buynow)) {
			...
			Assert.assertEquals(driver.findElement(By.xpath("//a[@class='btn buy' and text()='BUY NOW']")).getText(),"BUY NOW");
		...
		if(GeneralUtils.CheckElementdisplayed(btn_Checkout)) {
			...
			Assert.assertEquals(driver.findElement(By.xpath("//div[@class='cart-checkout' and text()='CHECKOUT']")).getText(),"CHECKOUT");
		...
		if(GeneralUtils.CheckElementdisplayed(txt_CVV)) {
			...
			if(GeneralUtils.CheckElementdisplayed(msg_InvalidCCNum)) {
				ExtentTestManager.getTest().log(LogStatus.INFO, "Exit from the purchase as an invalid credit card details are entered");
				return;
			}
